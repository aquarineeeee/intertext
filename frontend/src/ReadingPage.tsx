...
interface Sel { text: string; paragraphIndex: number; rect: DOMRect }
...
function TOCDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    <>
      {open && (
        <div
        ...
        <div className="pt-14 px-5 pb-8">
          ...
          <nav className="space-y-0.5">
            {CHAPTERS.map(ch => (
              <button
                key={ch.id}
                className={`w-full text-left px-3 py-2 rounded-sm transition-colors ${ch.current ? 'text-ink bg-surface' : 'text-mid reading-toc-item'}`}
                onClick={onClose}
              >
                <div className="text-xs text-faint mb-0.5" style={{ fontFamily: 'var(--font-ui)' }}>{ch.title}</div>
                <div className="text-sm" style={{ fontFamily: 'var(--font-ui)' }}>{ch.sub}</div>
              </button>
            ))}
          </nav>
        </div>
...
export default function App() {
  const [annotations, setAnnotations] = useState<Ann[]>(SEEDS)
  ...
  const sorted = [...annotations].sort((a, b) => {
    ...
    return (PS[a.paragraphIndex]?.indexOf(a.selectedText) ?? 0) - (PS[b.paragraphIndex]?.indexOf(b.selectedText) ?? 0)
  })
  ...
  const handleMouseUp = (e: React.MouseEvent) => {
    ...
    if (pi === -1) return
    setSel({ text, paragraphIndex: pi, rect })
  }
  ...
  const handleAction = (action: AnnType) => {
    ...
    if (action === 'bookmark') {
      const id = uid()
      setAnnotations(prev => [...prev, { id, type: 'bookmark', paragraphIndex: sel.paragraphIndex, selectedText: sel.text, note: '', messages: [], expanded: false }])
      setSel(null)
  ...
  const handleNoteSubmit = () => {
    if (!sel || !pending || !noteVal.trim()) return
    const id = uid()
    const newAnn: Ann = {
      id, type: pending,
      paragraphIndex: sel.paragraphIndex,
      selectedText: sel.text,
      note: pending === 'annotation' ? noteVal.trim() : '',
      messages: pending === 'discussion' ? [{ id: uid(), role: 'user', content: noteVal.trim() }] : [],
      expanded: false,
    }
    ...
    if (pending === 'discussion') {
      setAiTypingId(id)
      setTimeout(() => {
        setAnnotations(prev => prev.map(a => a.id === id ? { ...a, messages: [...a.messages, { id: uid(), role: 'ai', content: nextAI() }] } : a))
        setAiTypingId(null)
  ...
  const handleReply = (annId: string) => {
    ...
    setAiTypingId(annId)
    setTimeout(() => {
      setAnnotations(prev => prev.map(a => a.id === annId ? { ...a, messages: [...a.messages, { id: uid(), role: 'ai', content: nextFU() }] } : a))
      setAiTypingId(null)
    }, 1400)
  }
    ...
    <div className="reading-page-root h-full flex flex-col bg-cream" style={readingVars}>
      ...
      >
        ...
        <div className="reading-header-title flex-1 text-center leading-none">
          <span className="text-sm text-mid" style={{ fontFamily: 'var(--font-ui)' }}>{BOOK.title}</span>
          <span className="text-faint text-xs mx-2">·</span>
          <span className="reading-header-chapter text-xs text-faint" style={{ fontFamily: 'var(--font-ui)' }}>{BOOK.chapter}</span>
        </div>
