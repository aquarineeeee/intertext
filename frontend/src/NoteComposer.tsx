...
export default function NoteComposer({ books, fixedBook, onClose, onSave, saving = false, error, initialTitle = '', initialContent = '' }: NoteComposerProps) {
  const [title, setTitle] = useState(initialTitle)
  const [content, setContent] = useState(initialContent)
  const [selectedBook, setSelectedBook] = useState<NoteBook | null>(fixedBook || null)
  ...
  const mention = useMemo(() => {
    ...
    if (fixedBook || at < 0 || (at > 0 && !/\s/.test(prefix[at - 1]))) return null
  ...
  const handleContentChange = (value: string) => {
    ...
    const at = prefix.lastIndexOf('@')
    const valid = !fixedBook && at >= 0 && (at === 0 || /\s/.test(prefix[at - 1])) && !/[\r\n]/.test(prefix.slice(at + 1))
    setSuggestionsOpen(valid)
  }

  const chooseBook = (book: NoteBook) => {
    if (!mention) return
    const nextContent = content.slice(0, mention.at) + content.slice(mention.caret)
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    } else if (suggestionsOpen && suggestions.length > 0 && event.key === 'ArrowUp') {
      event.preventDefault()
    ...
    <div className="note-composer-backdrop" onClick={onClose}>
      <section className="note-composer" role="dialog" aria-modal="true" aria-label="新建笔记" onClick={event => event.stopPropagation()}>
        <header className="note-composer-header">
          <span>New note</span>
          <button type="button" onClick={onClose} disabled={saving} aria-label="关闭"><X size={17} /></button>
        </header>
        <input value={title} onChange={event => setTitle(event.target.value)} placeholder="标题（可选）" autoFocus />
        {selectedBook && <LinkedBook title={selectedBook.title} onRemove={fixedBook ? undefined : () => setSelectedBook(null)} />}
        <div className="note-composer-editor-wrap">
          <textarea
            ...
            placeholder={fixedBook ? '写下你的想法…' : '输入@ 关联书籍，按Tab自动补全'}
            rows={8}
          />
